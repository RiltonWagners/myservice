...
@else
<div class="pricing-header" style="padding-bottom: 5px; font-size: 16px;">
    <span><strong>Mostrando {{count($publications)}}{{count($publications) > 1 ? ' resultados' : ' resultado'}} em </strong></span>
    <span style="color: #fe5002;"><strong>{{$publications[0]->city->name}}/{{$publications[0]->city->state->uf}}</strong></span>
</div>
@endif
                        ...
                        <div class="media" style="height: auto; display: inline;">
                            <div class="media-body" style="padding: 5px; display: flex; margin: 10px;">
                                ...
                                <div>
                                    ...
                                    @if($loop->first)
                                    <a href="{{ route('publication.show', $publication->publication.id)}}" class="align-self-center mr-1" style="text-decoration: none;">
                                ...
                                <div style="margin-left: inherit;">
                                    <div>
                                        <a href="{{ route('publication.show', $publication->publication.id)}}" class="stretched-link" style="text-decoration: none; color: #4a4a4a; padding-right: 10px;">
                            ...
                            <div class="media-body media-body2">
                                <div class="media-body-description">
                                    <div class="media-body-description">
                                        <span class="mb-1 text-muted" style="font-size: 14px;">{{ implode(" ", array_slice(explode(" ", $publication->description), 0, 50))}}...</span>
                                    </div>
                                    <div class="media-body-city">
                                        <span class="mb-2 text" style="font-size: 14px;">
                                            ...
                                            <a href="{{ route('publication.show', $publication->publication.id)}}" class="stretched-link" style="text-decoration: none; color: #4a4a4a; padding-right: 10px;">{{$publication->city->name}}/{{$publication->city->state->uf}}</a></span>
                                    </div>
                                </div>
                            ...
                            <div class="media-body media-body-category">
                                <div class="media-body-category-item">
                                    <div class="media-body">
                                        <div><span style="font-size: 16px;" class="mb-0"><strong>{{$publication->service->category->name}}</strong></span></div>
...
<div class="d-md-none">
    ...
    <div class="card" style="margin-bottom: 10px;">
        <div class="media">
            ...
            @forelse($publication->PublicationImage as $i => $image)
            @if($loop->first)
            <a href="{{ route('publication.show', $publication->id)}}" class="align-self-center mr-1" style="text-decoration: none;">
            ...
            <div class="media-body" style="padding: 5px;">
                <div><span style="font-size: 12px;" class="mb-0"><strong>{{$publication->title}}</strong></span></div>
                <div><span class="mb-2 text-success" style="font-size: 12px;">{{$publication->service->category->name}}</span></div>
                <div><span class="mb-1 text-muted" style="font-size: 11px;">{{ implode(" ", array_slice(explode(" ", $publication->description), 0, 4))}}...</span></div>
                <p class="mb-auto"></p>
                <div>
                    <span class="mb-2 text-success" style="font-size: 11px;"><a href="{{ route('publication.show', $publication->id)}}" class="stretched-link" style="text-align: -webkit-right; text-decoration: none; color: #4a4a4a; float: right;padding-right: 10px;">{{$publication->city->name}}/{{$publication->city->state->uf}}</a></span>
                </div>
